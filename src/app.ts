...
app.get("/ename/:eventName", async (req, res) => {
    let targetName = req.params.eventName;
    try {
        let nameExists = await actionHander.eventNameAvailable(targetName);
        res.setHeader('Content-Type', 'application/json');
        res.send(JSON.stringify({taken: nameExists}));
    } catch (error) {
        res.status(500);
        res.setHeader('Content-Type', 'application/json');
        res.send(JSON.stringify({error: "Unable to determine availability"}));
        console.error(error);
    }
})
